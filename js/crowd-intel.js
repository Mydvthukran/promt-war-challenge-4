            ...
            <div class="card-body">
              ${crowdData.filter(z => z.status === 'critical' || z.status === 'high').map(z => `
                <div class="alert-card ${z.status === 'critical' ? 'alert-danger' : 'alert-warning'}" style="margin-bottom:var(--space-sm)">
                  <div class="alert-icon">${App.getIconSvg(z.status === 'critical' ? 'alert-octagon' : 'alert-triangle').replace('<svg', '<svg style="width:14px;height:14px;display:inline-block;vertical-align:text-bottom"')}</div>
